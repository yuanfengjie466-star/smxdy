...
| **🔐 API Key 内存存储** | 仅保存在内存，关闭网页即销毁，零泄露风险 |
| **🔒 本地密码保护** | SHA-256 + 随机盐 + 二次哈希，用于锁屏验证 |
| **🔐 自动锁屏** | 页面失焦/后台切换自动挂锁 |
...
2. **设置密码** - 设置一个本地隐私密码（用于锁屏保护，遗失无法找回）
3. **开始使用** - 输入问题即可获得三模型协作的精准回答
4. **自动锁屏** - 页面切换到后台或点击「锁屏」时自动挂锁
5. **关闭网页** - API Key 自动销毁，下次打开需重新输入
6. **退出登录** - 点击「退出」清除本地密码和聊天记录
...
| 数据 | 存储位置 | 说明 |
|------|---------|------|
| API Key | **内存 (RAM)** | 仅保存在 React state，关闭/刷新网页即销毁 |
| 密码哈希 | `localStorage` | SHA-256 + 随机盐 + 二次哈希 |
| 随机盐 | `localStorage` | 128位随机数 |

**安全特性：**
- ✅ API Key **不持久化**，关闭网页即销毁，无持久化后泄露风险
- ✅ API Key 通过 HTTP header 传递，后端不存储
